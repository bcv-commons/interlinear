 ...
 * a verse genuinely missing from one source, e.g. an alternate translation
 * that skips a verse). Pure proportional sync (syncScrollPanels above)
 * drifts permanently once that happens, because "fraction of the way
 * through the whole panel" no longer means the same verse on both sides,
 * and the error compounds for the rest of the chapter.
 *
 ...
 * back to the nearest verse number actually present there if that exact
 * one is missing. Self-correcting rather than cumulative: a mismatch only
 * ever throws off the instant it happens, not everything after it.
 *
 ...
 * pixel. In reverse, a short source verse only ever produces a small
 * pixel offset, so the moment the source crosses into its next verse, the
 * taller target has to snap forward by a whole verse's height in a single
 * jump instead of getting there gradually. Scaling by the anchor verse's
 * own height on each side keeps the mapping proportional (and thus the
 * motion smooth) regardless of which panel happens to be taller.
 */
