/**
 * Grammar/morphology code expansion, ported from
 * example/study-app/lib/ui/home/word_details_dialog/dialog_manager.dart.
 */

const MORPHOLOGY_CODES = {
...
export function expandGrammar(grammar) {
	const parts = grammar.split('-');
	let out = '';
	parts.forEach((part, i) => {
		const expanded = MORPHOLOGY_CODES[part] ?? part;
		if (i === 0) {
			out += expanded.charAt(0).toUpperCase() + expanded.slice(1);
		} else if (i === 1) {
			out += ': ' + expanded;
		} else {
			out += ', ' + expanded;
